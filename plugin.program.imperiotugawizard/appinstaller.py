...
AddonTitle="Imperio Tuga App Installer"
dialog       =  xbmcgui.Dialog()
AddonTitle3 = "[COLOR cyan][B][I]Imperio Tuga[/I][/B][/COLOR][COLOR grey] [B]FILE CLEANER[/B][/COLOR]"
...
def MENUSISTEM():
    addDir('ANDROID',BASEURL,23,ART+'android3.png',FANART,'')
    #addDir('WINDOWS',BASEURL,12,ART1+'windows3.png',FANART1,'')
    #addDir('LINUX',BASEURL,97,ART2+'linux3.png',FANART2,'')
    #addDir('REQUESTS',BASEURL,13,ART+'requests.png',FANART3,'')
    setView('list', 'MAIN')
...
def BUILDMENUDROID():
    addDir('APPS',BASEURL,25,ART+'apps.png',FANART,'')
    addDir('EMULATORS',BASEURL,26,ART+'emulators.png',FANART,'')
    addDir('ROMS',BASEURL,98,ART+'roms.png',FANART,'')
    setView('list', 'MAIN')
...
#def BUILDMENU1():
   # addDir('PROGRAMS',BASEURL,98,ART1+'programs.png',FANART,'')
    #addDir('ISOs',BASEURL,98,ART1+'isos.png',FANART,'')
    #setView('list', 'MAIN')
...
def APPINSTALL(name,url,description):
   ...
   if confirm.yesno(name,description,"","","GO BACK","DOWNLOAD"):
    ...
    dp.create("[COLOR cyan][B][I]Imperio Tuga[/I][/B][/COLOR][COLOR grey] [B]APP INSTALLER[/B][/COLOR]","DOWNLOADING...",name,'PLEASE WAIT...')
    lib=os.path.join(downloadpath, name+'.apk')
    downloader.download(url, lib, dp)
    addonfolder = xbmc.translatePath(os.path.join('special://','home'))
    dialog = xbmcgui.Dialog()
    dialog.ok("[COLOR cyan][B][I]Imperio Tuga[/I][/COLOR][COLOR grey] APP INSTALLER[/B][/COLOR]","DOWNLOAD COMPLETE.","","CLICK OK TO INSTALL " + name + "")
	#][NT3L]I[G3NC][# 
    xbmc.executebuiltin('StartAndroidActivity("","android.intent.action.VIEW","application/vnd.android.package-archive","file:'+lib+'")')
    time.sleep(4)
    CLEANUP()
    dialog.ok("[COLOR cyan][B][I]Imperio Tuga[/I][/COLOR][COLOR grey] APP INSTALLER[/B][/COLOR]"," "," ","THANKS FOR USING [COLOR cyan][I]Imperio Tuga[/I][/COLOR]APP INSTALLER")
    try: os.remove(lib)
    except: pass
...
def EMULATORINSTALL(name,url,description):
   ...
   if confirm.yesno(name,description,"","","GO BACK","DOWNLOAD"):
    downloadpath = xbmc.translatePath(os.path.join('special://home/addons','packages'))#
    dp = xbmcgui.DialogProgress()
    dp.create("[COLOR cyan][B][I]Imperio Tuga[/I][/COLOR][COLOR grey] EMULATOR INSTALLER[/B][/COLOR]","DOWNLOADING...",name,'PLEASE WAIT...')
    lib=os.path.join(downloadpath, name+'.apk')
    downloader.download(url, lib, dp)
    addonfolder = xbmc.translatePath(os.path.join('special://','home'))
    dialog = xbmcgui.Dialog()
    dialog.ok("[COLOR cyan][B][I]Imperio Tuga[/I][/COLOR][COLOR grey] EMULATOR INSTALLER[/B][/COLOR]","DOWNLOAD COMPLETE.","","CLICK OK TO INSTALL " + name + "")
	#][NT3L]I[G3NC][# 
    xbmc.executebuiltin('StartAndroidActivity("","android.intent.action.VIEW","application/vnd.android.package-archive","file:'+lib+'")')
    time.sleep(4)
    CLEANUP()
    dialog.ok("[COLOR cyan][B][I]Imperio Tuga[/I][/COLOR][COLOR grey] EMULATOR INSTALLER[/B][/COLOR]"," "," ","THANKS FOR USING [COLOR cyan][I]Imperio Tuga[/I][/COLOR] EMULATOR INSTALLER")
    try: os.remove(lib)
    except: pass
...
#def ADULTINSTALL(name,url,description):
 #  confirm=xbmcgui.Dialog()
  # if confirm.yesno(name,description,"","","GO BACK","DOWNLOAD"):
   # downloadpath = xbmc.translatePath(os.path.join('special://home/addons','packages'))#
    #dp = xbmcgui.DialogProgress()
    #dp.create("[COLOR cyan][B][I]Imperio Tuga[/I][/COLOR][COLOR grey] ADULT APP INSTALLER[/B][/COLOR]","DOWNLOADING...",name,'PLEASE WAIT...')
    #lib=os.path.join(downloadpath, name+'.apk')
    #downloader.download(url, lib, dp)
    #addonfolder = xbmc.translatePath(os.path.join('special://','home'))
    #dialog = xbmcgui.Dialog()
    #dialog.ok("[COLOR cyan][B][I]Imperio Tuga[/I][/COLOR][COLOR grey] ADULT APP INSTALLER[/B][/COLOR]","DOWNLOAD COMPLETE.","","CLICK OK TO INSTALL " + name + "")
	#][NT3L]I[G3NC][# 
    #xbmc.executebuiltin('StartAndroidActivity("","android.intent.action.VIEW","application/vnd.android.package-archive","file:'+lib+'")')
    #time.sleep(4)
    #CLEANUP()
    #dialog.ok("[COLOR cyan][B][I]Imperio Tuga[/I][/COLOR][COLOR grey] ADULT APP INSTALLER[/B][/COLOR]"," "," ","THANKS FOR USING [COLOR cyan][I]Imperio Tuga[/I][/COLOR] ADULT APP INSTALLER [COLOR cyan][B]][[/B]NT3L[B]][[/B]G3NC[B]][[/B][/COLOR]")
    #try: os.remove(lib)
    #except: pass
	...
	if os.path.exists(PACKAGES):
		try:	
			for root, dirs, files in os.walk(PACKAGES):
				...
				if file_count > 0:
					if dialog.yesno("[COLOR cyan][B][I]Imperio Tuga[/I][/B][/COLOR][COLOR grey] [B]FILE CLEANER[/B][/COLOR]", str(file_count) + " FILES FOUND","","WOULD YOU LIKE TO DELETE THE PACKAGE FILES?", nolabel='NO, CANCEL',yeslabel='YES, DELETE'):
...
def REQUESTS():
    dialog.ok("[COLOR cyan][B][I]Imperio Tuga[/I][/COLOR][COLOR grey] APP INSTALLER[/B][/COLOR]","[COLOR grey]PLEASE SEND ANY PACKAGE REQUESTS TO:[/COLOR]","[COLOR dodgerblue]FACEBOOK: https://www.facebook.com/groups/KodiCustomBuilds/ [/COLOR]","[COLOR dodgerblue]FACEBOOK: https://www.facebook.com/profile.php?id=100011845114961[/COLOR]")
...
def addDir(name,url,mode,iconimage,fanart,description):
        ...
        if mode==24 or mode==27 or mode==11 or mode==13:
