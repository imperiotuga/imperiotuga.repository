...
addon_id = 'plugin.program.imperiotugawizard'
ADDON = xbmcaddon.Addon(id=addon_id)
AddonID='plugin.program.imperiotugawizard'
...
PATH = "Imperio Tuga Wizard"            
BASEURL = "http://imperiotuga.comxa.com/"
H = 'http://'
skin         =  xbmc.getSkinDir()
EXCLUDES2     = ['plugin.program.imperiotugawizard','script.module.addon.common','packages']
EXCLUDES     = ['plugin.program.imperiotugawizard','script.module.addon.common']
...
CBPATH       =  xbmc.translatePath(os.path.join(USB,'Imperio Tuga Builds',''))
...
#Root menu of addon
def INDEX():
	addDir('Imperio Tuga Builds',BASEURL,20,ART+'imperiowiz.png',FANART,'')
...
def BUILDMENU():
    link = OPEN_URL('http://imperiotuga.comxa.com/wiz').replace('\n','').replace('\r','')
...
def facebook():
    TextBoxes('Imperio Tuga Wizard', 'Join the discussions at')
...
def pop():
    popup=SPLASH('Imperio.xml',ADDON.getAddonInfo('path'),'DefaultSkin',close_time=1)
    popup.doModal()
    del popup
#-----------------------------------------------------------------------------------------------------------------    
...
def COMMUNITY_BACKUP():
    guisuccess=1
    CHECK_DOWNLOAD_PATH()
    fullbackuppath = xbmc.translatePath(os.path.join(USB,'Imperio Custom Builds','My Builds',''))
    myfullbackup = xbmc.translatePath(os.path.join(USB,'Imperio Custom Builds','My Builds','my_full_backup.zip'))
    myfullbackupGUI = xbmc.translatePath(os.path.join(USB,'Imperio Custom Builds','My Builds','my_full_backup_GUI_Settings.zip'))
    ...
    exclude_dirs_full =  ['plugin.program.imperiotugawizard']
    exclude_files_full = ["xbmc.log","xbmc.old.log","kodi.log","kodi.old.log",'.DS_Store','.setup_complete','XBMCHelper.conf']
    exclude_dirs =  ['plugin.program.imperiotugawizard', 'cache', 'system', 'Thumbnails', "peripheral_data",'library','keymaps']
    ...
    message_header2 = "Creating Imperio Custom Build"
    message1 = "Archiving..."
    message2 = ""
...
def FIX_SPECIAL(url):
    dp.create("Imperio Tuga Custom Builds","Renaming paths...",'', 'Please Wait')
...
def ARCHIVE_FILE(sourcefile, destfile):
    ...
    for_progress = []
    ITEM =[]
    dp.create("Imperio Tuga Custom Builds","Archiving...",'', 'Please Wait')
    ...
    for base, dirs, files in os.walk(sourcefile):
        for file in files:
            ...
            if not 'temp' in dirs:
                if not 'plugin.program.imperiotugawizard' in dirs:
                   import time
                   FORCE= '01/01/1980'
...
#---------------------------------------------------------------------------------------------------
#Zip up tree
# "Imperio Tuga Custom Builds","Archiving...",'', 'Please Wait'
...
def RESTORE_COMMUNITY(name,url,video,description,skins,guisettingslink):
    ...
    try: os.rename(GUI,GUINEW) #Rename guisettings.xml to guinew.xml so we can edit without XBMC interfering.
        ...
        return
    choice = xbmcgui.Dialog().yesno(name, 'We highly recommend backing up your existing build before', 'installing any Imperio Tuga Custom Builds.', 'Would you like to perform a backup first?', nolabel='Backup',yeslabel='Install')
    if choice == 0:
        mybackuppath = xbmc.translatePath(os.path.join(USB,'Imperio Tuga Custom Builds','My Builds'))
    ...
    if choice4 == 0: return
    time.sleep(1)
    dp.create("Imperio Tuga Custom Builds","Downloading "+description +" build.",'', 'Please Wait')
    ...
    readfile.close()
    READ_ZIP(lib)
    dp.create("Imperio Tuga Custom Builds","Checking ",'', 'Please Wait')
...
#    if zip == '':
#        dialog.ok('Imperio Tuga Custom Builds Tool','You have not set your ZIP Folder.\nPlease update the addon settings and try again.','','')
#        ADDON.openSettings(sys.argv[0])
    path = xbmc.translatePath(os.path.join(zip,'testCBFolder'))
    if not os.path.exists(zip):
        dialog.ok('Imperio Tuga Custom Builds Tool','The download location you have stored does not exist .\nPlease update the addon settings and try again.','','')        
        ADDON.openSettings(sys.argv[0])
#---------------------------------------------------------------------------------------------------
...
def RESTORE_LOCAL_COMMUNITY():
    ...
    if exitfunction == 1:
        ...
        readfile.close()
        READ_ZIP(filename)
        dp.create("Imperio Tuga Custom Builds Tool","Checking ",'', 'Please Wait')
        dp.update(0,"", "Extracting Zip Please Wait")
        ...
        dialog.ok('Imperio Tuga Custom Builds Tool','Step 1 complete. Now please change the skin to','the one this build was designed for. Once done come back','to this addon and restore the guisettings_fix.zip')        
        xbmc.executebuiltin("ActivateWindow(appearancesettings)")
#---------------------------------------------------------------------------------------------------
...
def RESTORE_ZIP_FILE(url):
    ...
    if 'Backup' in name:
        ...
        import zipfile
        import sys
        dp.create("Imperio Tuga Custom Builds Tool","Backing Up",'', 'Please Wait')
        ...
        zipobj.close()
        dp.close()
        dialog.ok("Imperio Tuga Custom Builds Tool", "You Are Now Backed Up", '','')   
    else:
        ...
        if 'Backup' in name:
            killxbmc()
            dialog.ok("Imperio Tuga Custom Builds - Install Complete", 'To ensure the skin settings are set correctly XBMC will now', 'close. If XBMC doesn\'t close please force close (pull power', 'or force close in your OS - [COLOR=lime]DO NOT exit via XBMC menu[/COLOR])')
        else:
            dialog.ok("Imperio Tuga Custom Builds Tool", "You Are Now Restored", '','')        
#---------------------------------------------------------------------------------------------------
...
def RESTORE_BACKUP_XML(name,url,description):
    if 'Backup' in name:
        ...
        if 'guisettings.xml' in description:
            ...
            f.write(TO_READ)
            f.close()  
    dialog.ok("Imperio Tuga Custom Builds Tool", "", 'All Done !','')
#---------------------------------------------------------------------------------------------------
    ...
    # Count files and give option to delete
        if file_count >= 0:
            ...
            for d in dirs:
                shutil.rmtree(os.path.join(root, d))        
#---------------------------------------------------------------------------------------------------
#Function to do a full wipe. Thanks to kozz for working out how to add an exclude clause so Imperio Tuga Custom Builds addon_data and addon isn't touched.
def WipeXBMC():
    if skin!= "skin.confluence":
        dialog.ok('Imperio Tuga Custom Builds Tool','Please switch to the default Confluence skin','before performing a wipe.','')
        xbmc.executebuiltin("ActivateWindow(appearancesettings)")
        return
    else:
    ...
    if choice2 == 0:
        ...
        dp.create("Imperio Tuga Custom Builds Tool","Wiping Install",'', 'Please Wait')
        try:
    ...
    REMOVE_EMPTY_FOLDERS()
    REMOVE_EMPTY_FOLDERS()
    dialog.ok('Imperio Tuga Custom Builds Tool','Wipe Successful, please restart XBMC/Kodi for changes to take effect.','','')
...
#Thanks to kozz for working out how to add an exclude clause so Imperio Tuga Custom Builds addon_data and addon isn't touched.
def WipeInstall():
    if skin!= "skin.confluence":
        dialog.ok('Imperio Tuga Custom Builds Tool','Please switch to the default Confluence skin','before performing a wipe.','')
        xbmc.executebuiltin("ActivateWindow(appearancesettings)")       
...
def DELETEIVUEDB():
    ...
    dialog = xbmcgui.Dialog()
    dialog.ok("Imperio Tuga Maintenance", "Database Files Removed, Please reload TV Guide", "[COLOR white]Brought To You By Imperio Tuga[/COLOR]")
...
def WIZARD(name,url,description):
    ...
    path = xbmc.translatePath(os.path.join('special://home/addons','packages'))
    dp = xbmcgui.DialogProgress()
    dp.create("Imperio Tuga [COLOR white]Wizard[/COLOR]","Downloading ",'', 'Please Wait')
    lib=os.path.join(path, name+'.zip')
    try:
       os.remove(lib)
    except:
    ...
    extract.all(lib,addonfolder,dp)
    dialog = xbmcgui.Dialog()
    dialog.ok("Imperio Tuga [COLOR white]Wizard[/COLOR]", "To save changes you now need to force close Kodi, Press OK to force close Kodi")
    killxbmc()
...
def ADDONWIZARD(name,url,description):
    ...
    extract.all(lib,addonfolder,dp)
    dialog = xbmcgui.Dialog()
    dialog.ok("Imperio Tuga [COLOR white]Wizard[/COLOR]", "Please restart kodi for changes To Take Effect","[COLOR yellow]Brought To You By [COLOR white][B]Imperio Tuga[/B][/COLOR][/COLOR]")
            ...
            if file_count > 0:
                ...
                if dialog.yesno("Delete Package Cache Files", str(file_count) + " files found", "Do you want to delete them?"):
                    ...
                    for d in dirs:
                        shutil.rmtree(os.path.join(root, d))
                    dialog = xbmcgui.Dialog()
                    dialog.ok("Imperio Tuga [COLOR white]Wizard[/COLOR]", "Packages Successfuly Removed", "[COLOR yellow]Brought To You By [COLOR white][B]Imperio Tuga[/B][/COLOR][/COLOR]")
    except: 
        dialog = xbmcgui.Dialog()
        dialog.ok("Imperio Tuga [COLOR white]Wizard[/COLOR]", "Sorry we were not able to remove Package Files", "[COLOR yellow]Brought To You By [COLOR white][B]Imperio Tuga [/B][/COLOR][/COLOR]")
...
def deletecachefiles(url):
    ...
    dialog = xbmcgui.Dialog()
    dialog.ok("Imperio Tuga [COLOR white]Wizard[/COLOR]", " All Cache Files Removed", "[COLOR yellow]Brought To You By [COLOR white][B]Imperio Tuga [/B][/COLOR][/COLOR]")
...
def FRESHSTART(params):
    if skin!= "skin.confluence":
        dialog.ok('Imperio Tuga[COLOR=white] Wizard[/COLOR] ','Please switch to the default Confluence skin','before performing a wipe.','')
        xbmc.executebuiltin("ActivateWindow(appearancesettings)")
        return
    else:
    ...
    if choice2 == 0:
        ...
        dp.create("Imperio Tuga[COLOR=white] Wizard[/COLOR]","Wiping Install",'', 'Please Wait')
        try:
            for root, dirs, files in os.walk(HOME,topdown=True):
                dirs[:] = [d for d in dirs if d not in EXCLUDES]
                for name in files:
    ...
    REMOVE_EMPTY_FOLDERS()
    REMOVE_EMPTY_FOLDERS()
    dialog.ok('Imperio Tuga[COLOR=white] Wizard[/COLOR]','Wipe Successful, please restart XBMC/Kodi for changes to take effect.','','')
    killxbmc()
...
def IPCHECK(url='http://www.iplocation.net/',inc=1):
    ...
    for ip, region, country, isp in match:
        if inc <2: dialog=xbmcgui.Dialog(); dialog.ok('Imperio Tuga[COLOR gold] Wizard[/COLOR]',"[B][COLOR gold]Your IP Address is: [/COLOR][/B] %s" % ip, '[B][COLOR gold]Your IP is based in: [/COLOR][/B] %s' % country, '[B][COLOR gold]Your Service Provider is:[/COLOR][/B] %s' % isp)
        inc=inc+1
...
def UPDATEREPO():
    ...
    xbmc.executebuiltin( 'UpdateAddonRepos' )
    dialog = xbmcgui.Dialog()
    dialog.ok("Imperio Tuga[COLOR gold] Wizard[/COLOR]", '','                                 REFRESH SUCCESSFUL :)', "                          [COLOR yellow]Brought To You By [COLOR blue][B]Imperio Tuga[/B][/COLOR][/COLOR]")
    return
...
if mode==None or url==None or len(url)<1:
	...
	FRESHSTART(params)
       ...
       donation()
        ...
        print "############   Imperio Tuga Custom Builds Menu   #################"
        CB_Root_Menu()
